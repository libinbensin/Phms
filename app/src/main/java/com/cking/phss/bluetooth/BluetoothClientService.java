...
public class BluetoothClientService {
     ...
     * @param device  The BluetoothServer to connect
    ...
    /**
     * 这个函数是断开上次连接的通讯，不能发送MESSAGE_DISCONNECT消息，否则会造成
     * 上层认为通讯失败，其实接下来马上回去连接。
    ...
    private class ConnectThread extends Thread {
        ...
        public ConnectThread(BluetoothDevice device, boolean secure) {
            ...
            if (D) Log.d(TAG, "ConnectThread");

            // Get a BluetoothSocket for a connection with the
            // given BluetoothServer
            try {
//                if (secure) {
...
//                }
        ...
        public void run() {
            ...
            try {
                ...
                if (D) Log.d(TAG, "mmSocket.connect()");
                mmSocket.connect();
            } catch (IOException e) {
    ...
    private class ConnectedThread extends Thread {
        ...
        public void write(byte[] buffer) {
            if (D) Log.d(TAG, "ConnectThread, write");
            try {
