...
public class JktjLnpgTestPage01 extends LinearLayout implements OnClickListener {
    private JktjLnpgPage0101 mParent = null;
...
//	RelativeGroup mRadioBarAnswer = null;
    ...
    private int[] radioBtnIds = new int[] { R.id.radio_tzps_answer1, R.id.radio_tzps_answer2,
            R.id.radio_tzps_answer3, R.id.radio_tzps_answer4 };
	 ...
	 * @param context
	 */
    public JktjLnpgTestPage01(Context context, JktjLnpgPage0101 jktjLnpgPage0101, int sexCd) {
		super(context);
        this.mParent = jktjLnpgPage0101;
		this.sexCd=sexCd;
		init(context);
	}
	 ...
	 * @param attrs
	 */
	public JktjLnpgTestPage01(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context);
	}
	...
	private void init(Context context) {
        ...
        inflater.inflate(R.layout.fragment_health_lnpg_zilipg_2_layout, this);
				...
				if (MyApplication.getInstance().getSession()
					...
					if (cursor.moveToNext()) {// 说明有数据存在，那么更新
						SqliteOperater.getDbInstance().updateBySql(
                                "UPDATE " + LnpgField.TB_NAME + " SET " + LnpgField.ZILIPG
...
//        }
    ...
    private List<Topic> getTopicList() {
        ...
        for (int i = 0; i < Lnpg.ZILIPG_QUESTION.length; i++) {
            String question = Lnpg.ZILIPG_QUESTION[i];// 问题
            list.add(new Topic(question, 0, Lnpg.ZILIPG_FENSHU[i]));
        }
        return list;
    }
	...
	private class Topic {
        ...
        public Topic(String question, int answerIndex, int[] rate) {
			this.title = "";
			this.question = question;
            this.selection = new String[] { "可自理", "轻度依赖", "中度依赖", "不能自理" };
