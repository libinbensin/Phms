...
public class JktjLnpgTestPage02 extends LinearLayout implements OnClickListener {
    private JktjLnpgPage0201 mParent = null;
...
//	RelativeGroup mRadioBarAnswer = null;
	 ...
	 * @param context
	 */
    public JktjLnpgTestPage02(Context context, JktjLnpgPage0201 jktjLnpgPage0201, int sexCd) {
		super(context);
	 ...
	 * @param attrs
	 */
	public JktjLnpgTestPage02(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context);
	}
	...
	private void init(Context context) {
        ...
        inflater.inflate(R.layout.fragment_health_lnpg_yiyupg_2_layout, this);
				...
				if (MyApplication.getInstance().getSession()
					...
					if (cursor.moveToNext()) {// 说明有数据存在，那么更新
						SqliteOperater.getDbInstance().updateBySql(
                                "UPDATE " + LnpgField.TB_NAME + " SET " + LnpgField.YIYUPG
...
//        }
    ...
    private List<Topic> getTopicList() {
        ...
        for (int i = 0; i < Lnpg.YIYUPG_QUESTION.length; i++) {
            String question = Lnpg.YIYUPG_QUESTION[i];// 问题
            list.add(new Topic(question, 0, Lnpg.YIYUPG_FENSHU[i]));
        }
        return list;
    }
