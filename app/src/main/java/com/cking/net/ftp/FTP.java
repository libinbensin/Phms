...
import com.cking.net.ProtocolCommandSupport;
import com.cking.net.SocketClient;
import com.cking.net.io.CRLFLineReader;

/***
 * FtpService provides the basic the functionality necessary to implement your
 * own FtpService client.  It extends com.jinny.net.SocketClient since
 * extending TelnetClient was causing unwanted behavior (like connections
 * that did not time out properly).
 * <p>
 * To derive the full benefits of the FtpService class requires some knowledge
 * of the FtpService protocol defined in RFC 959.  However, there is no reason
 * why you should have to use the FtpService class.  The
 * {@link com.cking.net.ftp.FTPClient} class,
 * derived from FtpService,
 * implements all the functionality required of an FtpService client.  The
 * FtpService class is made public to provide access to various FtpService constants
 * and to make it easier for adventurous programmers (or those with
 * special needs) to interact with the FtpService protocol and implement their
 * own clients.  A set of methods with names corresponding to the FtpService
 ...
 * given time period (usually 900 seconds).  The FtpService class will detect a
 ...
 * system resources used by FtpService.  Before disconnecting, you may check the
 ...
 * @version $Id: FtpService.java 1230358 2012-01-12 01:51:02Z sebb $
 ***/

public class FTP extends SocketClient
{
    /*** The default FtpService data port (20). ***/
    public static final int DEFAULT_DATA_PORT = 20;
    /*** The default FtpService control port (21). ***/
    ...
    /***
     * A constant used to indicate a file is to be transfered as FtpService
     ...
     * FtpService control connection.  The default encoding is an
     * ASCII-compatible encoding.  Some FtpService servers expect other
     * encodings.  You can change the encoding used by an FtpService instance
     ...
     * from the FtpService control connection.  Do not access the control
     ...
     * to the FtpService control connection.  Do not access the control
    ...
    /***
     * The default FtpService constructor.  Sets the default port to
     * <code>DEFAULT_PORT</code> and initializes internal data structures
     * for saving FtpService reply information.
     ***/
    public FTP()
    {
        super();
    ...
    private void __getReply(boolean reportReply) throws IOException
    {
        ...
        if (_replyCode == FTPReply.SERVICE_NOT_AVAILABLE) {
            throw new FTPConnectionClosedException("FtpService response 421 received.  Server closed connection.");
        }
    }
    ...
    /**
     * Sets the character encoding used by the FtpService control connection.
     * Some FtpService servers require that commands be issued in a non-ASCII
    ...
    /***
     * Closes the control connection to the FtpService server and sets to null
    ...
    /***
     * Sends an FtpService command to the server, waits for a reply and returns the
     ...
     * @param args The arguments to the FtpService command.  If this parameter is
     *             set to null, then the command is sent with no argument.
     * @return The integer value of the FtpService reply code returned by the server
     ...
     * {@link #getReplyStrings  getReplyStrings }.
     * <p>
     * @param command  The FTPCommand constant corresponding to the FtpService command
     *                 to send.
     * @param args The arguments to the FtpService command.  If this parameter is
     *             set to null, then the command is sent with no argument.
     * @return The integer value of the FtpService reply code returned by the server
    ...
    /***
     * Sends an FtpService command with no arguments to the server, waits for a
    ...
    /***
     * Returns the integer value of the reply code of the last FtpService reply.
     * You will usually only use this method after you connect to the
     * FtpService server to check that the connection was successful since
     * <code> connect </code> is of type void.
     * <p>
     * @return The integer value of the reply code of the last FtpService reply.
     ***/
    ...
    /***
     * Fetches a reply from the FtpService server and returns the integer reply
     ...
     * method if you are implementing your own FtpService client or if you need to
     * fetch a secondary response from the FtpService server.
     * <p>
     * @return The integer value of the reply code of the fetched FtpService reply.
     * @exception FTPConnectionClosedException
     *      If the FtpService server prematurely closes the connection as a result
     *      of the client being idle or some other reason causing the server
     *      to send FtpService reply code 421.  This exception may be caught either
    ...
    /***
     * Returns the lines of text from the last FtpService server response as an array
     * of strings, one entry per line.  The end of line markers of each are
     * stripped from each line.
     * <p>
     * @return The lines of text from the last FtpService response as an array.
     ***/
    ...
    /***
     * Returns the entire text of the last FtpService server response exactly
     * as it was received, including all end of line markers in NETASCII
     * format.
     * <p>
     * @return The entire text from the last FtpService response as a String.
     ***/
    ...
    /***
     * A convenience method to send the FtpService USER command to the server,
    ...
    /**
     * A convenience method to send the FtpService PASS command to the server,
     * receive the reply, and return the reply code.
    ...
    /***
     * A convenience method to send the FtpService ACCT command to the server,
     * receive the reply, and return the reply code.
     * <p>
    ...
    /***
     * A convenience method to send the FtpService ABOR command to the server,
     * receive the reply, and return the reply code.
     * <p>
     * @return The reply code received from the server.
     * @exception FTPConnectionClosedException
    ...
    /***
     * A convenience method to send the FtpService CWD command to the server,
     * receive the reply, and return the reply code.
     * <p>
    ...
    /***
     * A convenience method to send the FtpService CDUP command to the server,
     * receive the reply, and return the reply code.
     * <p>
     * @return The reply code received from the server.
     * @exception FTPConnectionClosedException
    ...
    /***
     * A convenience method to send the FtpService QUIT command to the server,
     * receive the reply, and return the reply code.
     * <p>
     * @return The reply code received from the server.
     * @exception FTPConnectionClosedException
    ...
    /***
     * A convenience method to send the FtpService REIN command to the server,
     * receive the reply, and return the reply code.
     * <p>
     * @return The reply code received from the server.
     * @exception FTPConnectionClosedException
    ...
    /***
     * A convenience method to send the FtpService SMNT command to the server,
     * receive the reply, and return the reply code.
     * <p>
    ...
    /***
     * A convenience method to send the FtpService PORT command to the server,
     * receive the reply, and return the reply code.
     * <p>
    ...
    /***
     * A convenience method to send the FtpService EPRT command to the server,
     * receive the reply, and return the reply code.
     *
     * Examples:
     * <code>
    ...
    /***
     * A convenience method to send the FtpService PASV command to the server,
    ...
    public int pasv() throws IOException
    {
     ...
     /***
     * A convenience method to send the FtpService EPSV command to the server,
     * receive the reply, and return the reply code.  Remember, it's up
     * to you to interpret the reply string containing the host/port
     * information.
     * <p>
    ...
    /**
     * A convenience method to send the FtpService TYPE command for text files
    ...
    /**
     * A convenience method to send the FtpService TYPE command to the server,
     * receive the reply, and return the reply code.
     * <p>
     * @param fileType  The type of the file (one of the <code>FILE_TYPE</code>
     *              constants).
    ...
    /***
     * A convenience method to send the FtpService STRU command to the server,
     * receive the reply, and return the reply code.
     * <p>
    ...
    /***
     * A convenience method to send the FtpService MODE command to the server,
     * receive the reply, and return the reply code.
     * <p>
    ...
    /***
     * A convenience method to send the FtpService RETR command to the server,
     ...
     * level of access, use {@link com.cking.net.ftp.FTPClient}
    ...
    /***
     * A convenience method to send the FtpService STOR command to the server,
     * receive the reply, and return the reply code.  Remember, it is up
     * to you to manage the data connection.  If you don't need this low
     * level of access, use {@link com.cking.net.ftp.FTPClient}
     * , which will handle all low level details for you.
    ...
    /***
     * A convenience method to send the FtpService STOU command to the server,
     * receive the reply, and return the reply code.  Remember, it is up
     * to you to manage the data connection.  If you don't need this low
     * level of access, use {@link com.cking.net.ftp.FTPClient}
     * , which will handle all low level details for you.
     ...
     *                  the remote end of the transfer.  Some FtpService servers
     *                  require this.
     * @return The reply code received from the server.
     * @exception FTPConnectionClosedException
     *      If the FtpService server prematurely closes the connection as a result
    ...
    /***
     * A convenience method to send the FtpService APPE command to the server,
     * receive the reply, and return the reply code.  Remember, it is up
     * to you to manage the data connection.  If you don't need this low
     * level of access, use {@link com.cking.net.ftp.FTPClient}
     * , which will handle all low level details for you.
    ...
    /***
     * A convenience method to send the FtpService ALLO command to the server,
     * receive the reply, and return the reply code.
     * <p>
    ...
    /**
     * A convenience method to send the FtpService FEAT command to the server, receive the reply,
    ...
    /***
     * A convenience method to send the FtpService REST command to the server,
     * receive the reply, and return the reply code.
     * <p>
    ...
    /**
     * A convenience method to send the FtpService MFMT command to the server,
     * receive the reply, and return the reply code.
     * <p>
    ...
    /***
     * A convenience method to send the FtpService RNFR command to the server,
     * receive the reply, and return the reply code.
     * <p>
    ...
    /***
     * A convenience method to send the FtpService RNTO command to the server,
     * receive the reply, and return the reply code.
     * <p>
    ...
    /***
     * A convenience method to send the FtpService DELE command to the server,
     * receive the reply, and return the reply code.
     * <p>
    ...
    /***
     * A convenience method to send the FtpService RMD command to the server,
     * receive the reply, and return the reply code.
     * <p>
    ...
    /***
     * A convenience method to send the FtpService MKD command to the server,
     * receive the reply, and return the reply code.
     * <p>
    ...
    /***
     * A convenience method to send the FtpService PWD command to the server,
     * receive the reply, and return the reply code.
     * <p>
     * @return The reply code received from the server.
     * @exception FTPConnectionClosedException
    ...
    /***
     * A convenience method to send the FtpService LIST command to the server,
     * receive the reply, and return the reply code.  Remember, it is up
     * to you to manage the data connection.  If you don't need this low
     * level of access, use {@link com.cking.net.ftp.FTPClient}
     * , which will handle all low level details for you.
    ...
    /**
     * A convenience method to send the FtpService MLSD command to the server,
     * receive the reply, and return the reply code.  Remember, it is up
     * to you to manage the data connection.  If you don't need this low
     * level of access, use {@link com.cking.net.ftp.FTPClient}
     * , which will handle all low level details for you.
    ...
    /**
     * A convenience method to send the FtpService MLST command to the server,
     * receive the reply, and return the reply code.  Remember, it is up
     * to you to manage the data connection.  If you don't need this low
     * level of access, use {@link com.cking.net.ftp.FTPClient}
     * , which will handle all low level details for you.
    ...
    /***
     * A convenience method to send the FtpService NLST command to the server,
     * receive the reply, and return the reply code.  Remember, it is up
     * to you to manage the data connection.  If you don't need this low
     * level of access, use {@link com.cking.net.ftp.FTPClient}
     * , which will handle all low level details for you.
    ...
    /***
     * A convenience method to send the FtpService SITE command to the server,
     * receive the reply, and return the reply code.
     * <p>
    ...
    /***
     * A convenience method to send the FtpService SYST command to the server,
     * receive the reply, and return the reply code.
     * <p>
     * @return The reply code received from the server.
     * @exception FTPConnectionClosedException
    ...
    /***
     * A convenience method to send the FtpService STAT command to the server,
     * receive the reply, and return the reply code.
     * <p>
     * @return The reply code received from the server.
     * @exception FTPConnectionClosedException
    ...
    /***
     * A convenience method to send the FtpService HELP command to the server,
     * receive the reply, and return the reply code.
     * <p>
     * @return The reply code received from the server.
     * @exception FTPConnectionClosedException
    ...
    /***
     * A convenience method to send the FtpService NOOP command to the server,
     * receive the reply, and return the reply code.
     * <p>
     * @return The reply code received from the server.
     * @exception FTPConnectionClosedException
