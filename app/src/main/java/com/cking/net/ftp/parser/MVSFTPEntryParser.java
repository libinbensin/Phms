...
package com.cking.net.ftp.parser;
...
import com.cking.net.ftp.FTPClientConfig;
import com.cking.net.ftp.FTPFile;
 ...
 * @see com.cking.net.ftp.FTPFileEntryParser FTPFileEntryParser (for
 *      usage instructions)
 */
public class MVSFTPEntryParser extends ConfigurableFTPFileEntryParserImpl {
    ...
    static final String JES_LEVEL_2_LIST_REGEX = "(\\S+)\\s+" + // job
            ...
            "(\\S+).*" // rest ignored
    ;
    private static final String TAG = "MVSFTPEntryParser";
     ...
     * only two are usable for FtpService transfer.
     *
     ...
     * for the FtpService transfer.
     *
     * Any one beginning with either F or V can safely used by FtpService transfer. All
     ...
     *
     * The MVS ftp server supports a number of features via the FtpService interface.
     * The features are controlled with the FtpService command quote site filetype=<SEQ|JES|DB2>
    ...
    /**
     * Parses a line of an z/OS - MVS FtpService server file listing and converts it
    ...
    private boolean parseFileList(FTPFile file, String entry) {
        if (matches(entry)) {
            ...
            file.setName(name);
            Log.i(TAG, "MVSFTPEntryParser************************************* name ================= " +name);
            // DSORG
            if ("PS".equals(dsorg)) {
