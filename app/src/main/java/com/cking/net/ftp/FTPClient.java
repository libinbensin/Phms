...
import com.cking.net.MalformedServerReplyException;
import com.cking.net.ftp.parser.DefaultFTPFileEntryParserFactory;
import com.cking.net.ftp.parser.FTPFileEntryParserFactory;
import com.cking.net.ftp.parser.MLSxEntryParser;
import com.cking.net.io.CRLFLineReader;
import com.cking.net.io.CopyStreamAdapter;
import com.cking.net.io.CopyStreamEvent;
import com.cking.net.io.CopyStreamListener;
import com.cking.net.io.FromNetASCIIInputStream;
import com.cking.net.io.ToNetASCIIOutputStream;
import com.cking.net.io.Util;

/***
 * FTPClient encapsulates all the functionality necessary to store and
 * retrieve files from an FtpService server.  This class takes care of all
 * low level details of interacting with an FtpService server and provides
 * a convenient higher level interface.  As with all classes derived
 * from {@link com.cking.net.SocketClient},
 * you must first connect to the server with
 * {@link com.cking.net.SocketClient#connect  connect }
 * before doing anything, and finally
 * {@link com.cking.net.SocketClient#disconnect  disconnect }
 * after you're completely finished interacting with the server.
 * Then you need to check the FtpService reply code to see if the connection
 ...
 *        ftp.disconnect();
 *        System.err.println("FtpService server refused connection.");
 *        System.exit(1);
 *      }
 *      ... // transfer files
 *      ftp.logout();
 ...
 * FtpService command methods in FTPClient is such that they either return a
 * boolean value or some other value.
 * The boolean methods return true on a successful completion reply from
 * the FtpService server and false on a reply resulting in an error condition or
 * failure.  The methods returning a value other than boolean return a value
 * containing the higher level data produced by the FtpService command, or null if a
 * reply resulted in an error condition or failure.  If you want to access
 * the exact FtpService reply code causing a success or failure, you must call
 * {@link com.cking.net.ftp.FTP#getReplyCode  getReplyCode } after
 * a success or failure.
 * <p>
 * The default settings for FTPClient are for it to use
 * <code> FtpService.ASCII_FILE_TYPE </code>,
 * <code> FtpService.NON_PRINT_TEXT_FORMAT </code>,
 * <code> FtpService.STREAM_TRANSFER_MODE </code>, and
 * <code> FtpService.FILE_STRUCTURE </code>.  The only file types directly supported
 * are <code> FtpService.ASCII_FILE_TYPE </code> and
 * <code> FtpService.BINARY_FILE_TYPE </code>.  Because there are at least 4
 ...
 * <code> FtpService.NON_PRINT_TEXT_FORMAT </code>,
 * <code> FtpService.STREAM_TRANSFER_MODE </code>, and
 * <code> FtpService.FILE_STRUCTURE </code> are the only supported formats,
 ...
 * <p>
 * You should keep in mind that the FtpService server may choose to prematurely
 * close a connection if the client has been idle for longer than a
 * given time period (usually 900 seconds).  The FTPClient class will detect a
 * premature FtpService server connection closing when it receives a
 * {@link com.cking.net.ftp.FTPReply#SERVICE_NOT_AVAILABLE FTPReply.SERVICE_NOT_AVAILABLE }
 *  response to a command.
 * When that occurs, the FtpService class method encountering that reply will throw
 * an {@link com.cking.net.ftp.FTPConnectionClosedException}
 * .
 ...
 * {@link com.cking.net.ftp.FTPConnectionClosedException}
 ...
 * {@link com.cking.net.ftp.FTP#getReplyCode  getReplyCode },
 * {@link com.cking.net.ftp.FTP#getReplyString  getReplyString },
 * and
 * {@link com.cking.net.ftp.FTP#getReplyStrings  getReplyStrings}.
 ...
 * can also throw a
 * {@link com.cking.net.MalformedServerReplyException}
 * , which is a subclass
 ...
 * from com.jinny.net.FTPFileEntryParser:
 * <pre>
 *    FTPClient f = new FTPClient();
 *    f.connect(server);
 *    f.login(username, password);
 ...
 * FtpService servers know that the control connection is in use, so won't close it through lack of activity,
 ...
 * @see FTPClientConfig
 *
 * @see com.cking.net.MalformedServerReplyException
 **/
...
implements Configurable
{
     ...
     * @since 3.0
     */
    public static final String FTP_SYSTEM_TYPE = "com.jinny.net.ftp.systemType";
     ...
     * @since 3.1
     */
    public static final String FTP_SYSTEM_TYPE_DEFAULT = "com.jinny.net.ftp.systemType.default";
     ...
     * <pre>
     * Plan 9=Unix
     * OS410=com.jinny.net.ftp.parser.OS400FTPEntryParser
     * </pre>
     *
     * @since 3.0
     */
    ...
    /***
     * A constant indicating the FtpService session is expecting all transfers
    ...
    // Most FtpService servers don't seem to support concurrent control and data connection usage
     ...
     * set to <code> FtpService.ASCII_FILE_TYPE </code>, the
     * file format set to <code> FtpService.NON_PRINT_TEXT_FORMAT </code>,
     * the file structure set to <code> FtpService.FILE_STRUCTURE </code>, and
     * the transfer mode set to <code> FtpService.STREAM_TRANSFER_MODE </code>.
     * <p>
    ...
    throws IOException
    {
        ...
        return new com.cking.net.io.SocketOutputStream(socket, output);
    }


    /**
     * Establishes a data connection with the FtpService server, returning
     ...
     * <p>
     * @param command  The int representation of the FtpService command to send.
     * @param arg The arguments to the FtpService command.  If this parameter is
     *             set to null, then the command is sent with no argument.
     * @return A Socket corresponding to the established data connection.
     *         Null is returned if an FtpService protocol error is reported at
     ...
     * mode connections also cause a local PORT command to be issued.
     * <p>
     * @param command  The text representation of the FtpService command to send.
     * @param arg The arguments to the FtpService command.  If this parameter is
     *             set to null, then the command is sent with no argument.
     * @return A Socket corresponding to the established data connection.
     *         Null is returned if an FtpService protocol error is reported at
    ...
    throws IOException
    {
        ...
        { // We must be in PASSIVE_LOCAL_DATA_CONNECTION_MODE
            ...
            // E.g. if FtpService server has a static PASV address (external network)
     ...
     *
     * @see com.cking.net.ftp.parser.FTPFileEntryParserFactory
     * @see com.cking.net.ftp.parser.DefaultFTPFileEntryParserFactory
     */
    ...
    /***
     * Closes the connection to the FtpService server and restores
    ...
    /***
     * Login to the FtpService server using the provided username and password.
     * <p>
     * @param username The username to login under.
     * @param password The password to use.
     * @return True if successfully completed, false if not.
     * @exception FTPConnectionClosedException
     *      If the FtpService server prematurely closes the connection as a result
     *      of the client being idle or some other reason causing the server
     *      to send FtpService reply code 421.  This exception may be caught either
    ...
    /***
     * Login to the FtpService server using the provided username, password,
    ...
    /***
     * Logout of the FtpService server by sending the QUIT command.
     * <p>
     * @return True if successfully completed, false if not.
     * @exception FTPConnectionClosedException
     *      If the FtpService server prematurely closes the connection as a result
    ...
    /***
     * Change the current working directory of the FtpService session.
     * <p>
    ...
    /***
     * Issue the FtpService SMNT command.
     * <p>
    ...
    /***
     * Reinitialize the FtpService session.  Not all FtpService servers support this
     * command, which issues the FtpService REIN command.
     * <p>
     * @return True if successfully completed, false if not.
     * @exception FTPConnectionClosedException
     *      If the FtpService server prematurely closes the connection as a result
     ...
     * with the FtpService server is conducted, but this causes all future data
     * transfers to require the FtpService server to connect to the client's
     ...
     * 
     * Useful for FtpService Client behind Firewall NAT.
     * <p>
     ...
     * <code> FtpService.ASCII_FILE_TYPE </code>, <code> FtpService.BINARY_FILE_TYPE</code>,
     * etc.  The file type only needs to be set when you want to change the
     * type.  After changing it, the new type stays in effect until you change
     * it again.  The default file type is <code> FtpService.ASCII_FILE_TYPE </code>
     ...
     * one of  <code> FtpService.ASCII_FILE_TYPE </code>,
     * <code> FtpService.BINARY_FILE_TYPE </code>, etc.  The file type only needs to
     ...
     * is <code> FtpService.ASCII_FILE_TYPE </code> if this method is never called.
     * The format should be one of the FtpService class <code> TEXT_FORMAT </code>
     * constants, or if the type is <code> FtpService.LOCAL_FILE_TYPE </code>, the
     * format should be the byte size for that type.  The default format
     * is <code> FtpService.NON_PRINT_TEXT_FORMAT </code> if this method is never
     * called.
     * <p>
     * <b>N.B.</b> currently calling any connect method will reset the mode to
     * ACTIVE_LOCAL_DATA_CONNECTION_MODE.
     ...
     * <code> FtpService.FILE_STRUCTURE </code> if this method is never called.
     * <p>
     * @param structure  The structure of the file (one of the FtpService class
     ...
     * <code> FtpService.STREAM_TRANSFER_MODE </code> if this method is never called.
     * <p>
     * @param mode  The new transfer mode to use (one of the FtpService class
     ...
     * The other server must have had a <code> remoteRetrieve </code> issued
     * to it by another FTPClient.  Many FtpService servers require that a base
     ...
     * entire sequence of FtpService commands to complete a transaction.  These
     ...
     *      to send FtpService reply code 421.  This exception may be caught either
     *      as an IOException or independently as itself.
     * @exception com.cking.net.io.CopyStreamException  
    ...
    throws IOException
    {
        ...
        return new com.cking.net.io.SocketInputStream(socket, input);
    }
     ...
     * from the given offset.  This will only work on FtpService servers supporting
     * the REST comand for the stream transfer mode.  However, most FtpService
    ...
    /***
     * Deletes a file on the FtpService server.
     * <p>
    ...
    /***
     * Removes a directory on the FtpService server (if empty).
     * <p>
    ...
    /***
     * Creates a new subdirectory on the FtpService server in the current directory
     ...
     * FtpService server.  FTPClient will remember the value and return the
    ...
    public String getSystemType() throws IOException
    {
        ...
        // in practice FtpService servers deviate, so we soften the condition to
    ...
    /***
     * Sends a NOOP command to the FtpService server.  This is useful for preventing
     * server timeouts.
     * <p>
     * @return True if successfully completed, false if not.
     * @exception FTPConnectionClosedException
     ...
     * if the FtpService server returned a positive completion code, otherwise
     * null is returned (the FtpService server returned a 550 error No files found.).
     ...
     * @exception FTPConnectionClosedException
     *                   If the FtpService server prematurely closes the connection
     *                   as a result of the client being idle or some other
     *                   reason causing the server to send FtpService reply code 421.
     ...
     *                   from the server.
     * @exception com.cking.net.ftp.parser.ParserInitializationException
     ...
     *                   com.jinny.net.ftp.FTPFileEntryParser
     ...
     * @see com.cking.net.ftp.parser.DefaultFTPFileEntryParserFactory
     * @see com.cking.net.ftp.parser.FTPFileEntryParserFactory
     * @see com.cking.net.ftp.FTPFileEntryParser
     */
    ...
    /***
     * Issue the FtpService STAT command to the server.
     * <p>
    ...
    /***
     * Issue the FtpService STAT command to the server for a given pathname.  This
    ...
    /**
     * Issue the FtpService MDTM command (not supported by all servers to retrieve the last
     ...
     * GMT, but not all FtpService servers honour this.
     *
    ...
    /**
     * Issue the FtpService MFMT command (not supported by all servers) which sets the last
     ...
     *
     * An FtpService server would indicate its support of this feature by including "MFMT"
